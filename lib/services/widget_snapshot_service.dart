...
/// empty → daily focus (completed-one 5min | all-daily-done | in-progress) → long-term focus (hour slots) → end of day (11pm-5am).
class WidgetSnapshotService {
  ...
  Future<WidgetSnapshot> generateSnapshot({
    ...
    // End of day (11pm to 5am)
    if (now.hour >= AppConstants.endOfDayStartHour || now.hour < AppConstants.endOfDayEndHour) {
      return _snapshotFor(
        now: now,
